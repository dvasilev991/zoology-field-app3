const STORAGE_KEY = 'turilik_field_records_v3';
...
function renderCoverPointGrid() {
  ...
  grid.innerHTML = arms.map(a => {
    const cells = coverPoints.slice(a.start, a.end).map((key, j) => {
      ...
      return `<div class="pointcell ${cat ? cat.cls : 'empty'}" data-index="${a.start + j}" title="${a.key}${j + 1}: ${cat ? cat.label : 'празно'}">${cat ? cat.short : (j + 1)}</div>`;
    }).join('');
...
function renderAllCoverUI() { renderTransectButtons(); renderCoverPointGrid(); renderCoverProgressOnly(); }
